...
def get_args(help_text):
    parser = argparse.ArgumentParser(help_text)
    parser.add_argument('argv', default='', nargs='?', help='文字列、部分一致。または番号。')
    parser.add_argument('-n', '--nocopy', action='store_true', help='指定するとコピーしない')
    return parser.parse_args()


def main():
    help_text = "よく使うフレーズを表示、コピーする。\n"
    for index, key in enumerate(list(text_dict.keys())):
        help_text += f">{str(index).zfill(len(str(len(list(text_dict.keys())))))}: {str(key)}" + '\n'
    args = get_args(help_text)
    argv = args.argv
    nocopy = args.nocopy
    ...
    result_text = ""
    if argv.isdecimal():
        if int(argv) > len(text_dict.keys())-1:
             print("list index out of range")
             exit()
        target_key, result_text = list(text_dict.items())[int(argv)]

    else:
        target_keyl = [key for key in list(text_dict.keys()) if argv in key]
        if len(target_keyl) < 1:
            print('not found')
            exit()
        target_key = target_keyl[0]
        result_text = text_dict.get(target_key)
